const TweetMockup = ({ content }) => {
  const allTweets = [
    { text: content.hook, isHook: true },
    ...content.thread.map((t) => ({ text: t, isHook: false })),
  ];

  return (
    <div className="space-y-0">
      {allTweets.map((tweet, i) => (
        <div key={i} className="bg-white border border-gray-200 first:rounded-t-2xl last:rounded-b-2xl px-4 py-3 relative">
          {/* Thread connector line */}
          {i < allTweets.length - 1 && (
            <div className="absolute left-[30px] bottom-0 w-0.5 h-3 bg-gray-200 translate-y-full z-10" />
          )}

          <div className="flex gap-3">
            {/* Avatar */}
            <div className="shrink-0">
              <div className="w-10 h-10 rounded-full bg-hampton-primary flex items-center justify-center text-white font-bold text-sm font-body">
                SP
              </div>
            </div>
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-1.5 mb-0.5">
                <span className="font-bold text-sm text-hampton-text font-body">Sam Parr</span>
                ...
                <span className="text-hampton-text/40 text-sm font-body">@TheSamParr</span>
              </div>

              <p className="text-sm text-hampton-text font-body whitespace-pre-line leading-relaxed">
                {tweet.text}
              </p>

              {/* Engagement bar on last tweet */}
              {i === allTweets.length - 1 && (
                <div className="flex items-center gap-6 mt-3 pt-2 border-t border-gray-100 text-hampton-text/30">
